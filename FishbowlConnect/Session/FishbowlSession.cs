...
namespace FishbowlConnect
{
    ...
    public partial class FishbowlSession : IDisposable, INotifyPropertyChanged
    {
        ...
        #region Product
        [Obsolete("Throws Error, use ExecuteQueryRq instead")]
        public async Task<List<ProductListInfo>> GetProductList(string Filter)
        {
            //filter the returned product results on product number for now, add description and others later
            List<ProductListInfo> ProductList = new List<ProductListInfo>();

            string WhereClause = "AND Product.ActiveFlag = 1";
            if (Filter != null)
            {
                WhereClause += @" AND UPPER(Product.Num) Like '%" + Filter.ToUpper() + "%'";
            }
            ...
            ExecuteQueryRqType ProductListRq = new ExecuteQueryRqType();
            ...
            ProductListRq.Query = @"Select product.num, product.description, product.price as retailprice, product.weight,
                                        product.len, product.width, product.height, tier2adjpricing.roundedprice as DistPrice

                                        from product
                                        left join tier2adjpricing on 
                                        (product.id = tier2adjpricing.productid and tier2adjpricing.prcustomerincltypeid = 3)
                                    Where " +
                                    WhereClause.Substring(4);


            ExecuteQueryRsType ProductListRs = await this.IssueXMLRequestAsync<ExecuteQueryRsType>(ProductListRq);

            //if (ProductListRs.statusCode != "1000")
            //{
            //    throw new System.Exception(ProductListRs.statusCode + " - " + Utilities.StatusCodeMessage(ProductListRs.statusCode));
            //}
            //else
            //{

            for (int i = 1; i < ProductListRs.Rows.Count; i++)
            {
                MatchCollection FieldValues = Regex.Matches(ProductListRs.Rows[i], "(\"([^\"]*)\"|([^,]*))(,|$)");
                ...
                //}
                ProductList.Add(new ProductListInfo
                {
                    ProductNumber = FieldValues[0].Groups[2].Value,
                    ProductDesc = FieldValues[1].Groups[2].Value,
                    RetailPrice = Convert.ToDouble(FieldValues[2].Groups[2].Value),
                    DistPrice = Convert.ToDouble(FieldValues[3].Groups[2].Value),
                    Weight = FieldValues[4].Groups[2].Value,
                    Length = FieldValues[5].Groups[2].Value,
                    Width = FieldValues[6].Groups[2].Value,
                    Height = FieldValues[7].Groups[2].Value,
                    CaseQty = FieldValues[8].Groups[2].Value,
                    ProductImage = FieldValues[9].Groups[2].Value
        ...
        {
            ProductGetRqType ProductGetRq = new ProductGetRqType();

            ProductGetRq.Number = ProductNumber;
            ProductGetRq.GetImage = GetImage.ToString();

            ProductGetRsType ProductGetRs = await this.IssueXMLRequestAsync<ProductGetRsType>(ProductGetRq);
            //ProductGetRs ProductGetRs = await this.IssueJsonRequestAsync<ProductGetRs>(ProductGetRq);


            //if (ProductGetRs.statusCode != "1000")
            //{
            //    throw new System.Exception(ProductGetRs.statusCode + " - " + Utilities.StatusCodeMessage(ProductGetRs.statusCode));
            //}
        ...
        public async Task<bool> SaveNewProduct(Product newProduct)
        {
            ...
            //if (productSaveRs.statusCode!= "1000")
            //{
            //    throw new Exception(productSaveRs.statusCode + " - " + Utilities.StatusCodeMessage(productSaveRs.statusCode));
            //}
            return true;
        ...
        [Obsolete("Use new json version, not made yet")]
        public async Task SaveProductPriceAndUpc(Product product)
        {
            ...
            ProductImportRq.Rows = new Rows();
            ProductImportRq.Rows.Row = new List<string>();
            ProductImportRq.Rows.Row.Add(ImportHeaders.IMPORT_PRODUCT_HEADER_PRICE_UPC_ONLY);
